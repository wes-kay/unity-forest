...
namespace SoftKitty
{
    public class EntityManagerObject_SettingsProvider : AssetSettingsProvider
    {
        private string searchContext;
        private VisualElement rootElement;
        public static EntityManagerObject CurrentSettings
        {
            get
            {
                if (SGD_SettingsProvider.CurrentSettings == null) return null;
                return  SGD_SettingsProvider.CurrentSettings.GetData<EntityManagerObject>();
            }
        }

        public EntityManagerObject_SettingsProvider()
       : base("Project/SoftKitty/Entity Manager", () => CurrentSettings)
        {
            keywords = GetSearchKeywordsFromGUIContentProperties<EntityManagerObject>();
        }
        ...
        {
            const string message = "You have no <color=#22FFAA>Entity Manager Data Object</color> Assigned. Please assign one in <color=#22AAFF>General Data Settings</color>.\n" +
            ...
            EditorUtils.BoxInfo("  Entity Manager Object in Project", 0);
            var filter = $"t:{typeof(EntityManagerObject).FullName}";
            ...
            if (GUILayout.Button("Auto Create & Assign"))
            {
                ...
                EntityManagerObject _newObj = new EntityManagerObject();
                string _basepath = EditorUtils.DataPath + "EntityManagerObject";
        ...
        [SettingsProvider]
        private static SettingsProvider CreateProjectSettingsMenu() => new EntityManagerObject_SettingsProvider();
