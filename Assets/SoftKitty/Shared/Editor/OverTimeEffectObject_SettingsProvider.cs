...
namespace SoftKitty
{
    public class OverTimeEffectObject_SettingsProvider : AssetSettingsProvider
    {
        private string searchContext;
        private VisualElement rootElement;
        public static OverTimeEffectObject CurrentSettings
        {
            get
            {
                if (SGD_SettingsProvider.CurrentSettings == null) return null;
                return  SGD_SettingsProvider.CurrentSettings.GetData<OverTimeEffectObject>();
            }
        }

        public OverTimeEffectObject_SettingsProvider()
       : base("Project/SoftKitty/SubData - Over Time Effect", () => CurrentSettings)
        {
            keywords = GetSearchKeywordsFromGUIContentProperties<OverTimeEffectObject>();
        }
        ...
        {
            const string message = "You have no <color=#22FFAA>Over Time Effect Data Object</color> Assigned. Please assign one in <color=#22AAFF>General Data Settings</color>.\n" +
                "You can create one by right click in a folder in the project panel, select in the context menu:\n" +
                "'<color=#22AAFF>Create > SoftKitty > Data Objects > Over Time Effect Data</color>'\n" +
            ...
            EditorUtils.BoxInfo("  Over Time Effect Object in Project", 0);
            var filter = $"t:{typeof(OverTimeEffectObject).FullName}";
            ...
            if (GUILayout.Button("Auto Create & Assign"))
            {
                ...
                OverTimeEffectObject _newObj = new OverTimeEffectObject();
                string _basepath = EditorUtils.DataPath + "OverTimeEffectObject";
        ...
        [SettingsProvider]
        private static SettingsProvider CreateProjectSettingsMenu() => new OverTimeEffectObject_SettingsProvider();
